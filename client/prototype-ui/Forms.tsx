...
function CardInput({ value, update }: BaseProps<T.Card>) {
  return (
    <div className="gap-2 p-2 ml-2 border border-gray-300 rounded-md">
      <div className="col-span-6 sm:col-span-4">
        ...
        <IntInput value={ value.value } update={(v) => update({ ...value, value: v })} />
      </div>
      <div className="col-span-6 sm:col-span-4">
...
function PlayerStateInput({ value, update }: BaseProps<T.PlayerState>) {
  return (
    <div className="gap-2 p-2 ml-2 border border-gray-300 rounded-md">
      <div className="col-span-6 sm:col-span-4">
        <label className="block mb-1 text-sm font-medium text-gray-700">hand</label>
        <ArrayInput<T.Card> value={ value.hand } update={(v) => update({ ...value, hand: v })} initialVal={ T.Card.default() }>
          {(value, update) =>
      ...
      <div className="col-span-6 sm:col-span-4">
        ...
        <ArrayInput<T.UserId> value={ value.players } update={(v) => update({ ...value, players: v })} initialVal={ "" }>
          {(value, update) =>
      ...
      <div className="col-span-6 sm:col-span-4">
        <label className="block mb-1 text-sm font-medium text-gray-700">turn</label>
        <StringInput value={ value.turn } update={(v) => update({ ...value, turn: v })} />
      </div>
      <div className="col-span-6 sm:col-span-4">
        <label className="block mb-1 text-sm font-medium text-gray-700">pile</label>
        <OptionalInput<T.Card> value={ value.pile } update={(v) => update({ ...value, pile: v })} initialVal={ T.Card.default() }>
          {(value, update) =>
            <CardInput value={value} update={update} />
          }
        </OptionalInput>
      ...
      <div className="col-span-6 sm:col-span-4">
        <label className="block mb-1 text-sm font-medium text-gray-700">winner</label>
        <OptionalInput<T.UserId> value={ value.winner } update={(v) => update({ ...value, winner: v })} initialVal={ "" }>
          {(value, update) =>
            <StringInput value={value} update={update} />
          }
        </OptionalInput>
                ...
                <div className="flex flex-col h-full py-6 overflow-y-scroll bg-white shadow-xl">
                  ...
                  <div className="relative flex-1 px-4 mt-6 sm:px-6">
                    <div className="forms">
                      ...
                      <MethodForm<IPlayCardRequest>
                        method="playCard"
                        submit={connection.playCard.bind(connection)}
                        initialize={ IPlayCardRequest.default }
                      >
                        {(value, update) => (
                          <div className="grid grid-cols-6 gap-2 pl-2 ml-2 border-l border-gray-300">
                            <div className="col-span-6 sm:col-span-4">
                              <label className="block mt-2 mb-1 text-sm font-medium text-gray-700">card</label>
                              <CardInput value={ value.card } update={(v) => update({ ...value, card: v })} />
                            </div>
                          </div>
                        )}
                      </MethodForm>
                      <MethodForm<IDrawCardRequest>
                        method="drawCard"
                        submit={connection.drawCard.bind(connection)}
                        initialize={ IDrawCardRequest.default }
                      >
                        {(value, update) => (
                          <div className="grid grid-cols-6 gap-2 pl-2 ml-2 border-l border-gray-300">
                          </div>
